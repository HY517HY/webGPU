...
A= n/(n-f), B = nf/(n-f)。
# 视图矩阵的推导
关于视图矩阵，其实就是重构坐标系，主要实现两个功能：  
1.以相机位置作为原点；
2.根绝右手系重构坐标轴，使得target - eye为z轴方向；
详细推导可见：https://juejin.cn/post/7093131978207133732  
这里可能存在的困惑：
1.为什么要翻转？  
翻转是因为如果相机朝向为-z轴，为了保证相机实际看到的页面和我们希望的画面一直，需要多世界坐标的z坐标进行“翻转”。
2.平移项是啥意思？
平移项就是使得相机位置变为原点位置，因为视图的坐标系的x,y轴与世界空间的x,y轴正向对应的，所以减去eye在相机坐标系x，y轴上的投影刚好移动到“原点”位置，z轴用+
主要是由于z轴需要翻转。
